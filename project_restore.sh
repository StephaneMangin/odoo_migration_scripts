...
#                        +------------------+
#
# Full process from odoo migration
##################################
# From Odoo migrated databasxe
# wget -q --show-progress --output-document /tmp/database.zip https://legacy-upgrade.odoo.com/database/eu1/XXXXX/XXXXXXXXXXXXXXXXXXXXXX==/upgraded/archive
# unzip -q /tmp/database.zip -d /tmp/database.zip.extract
# rm -Rf /tmp/database.zip
# docker-compose run --rm odoo dropdb odoodb_template
# docker-compose run --rm odoo createdb odoodb_template
# PGPASSWORD=odoo psql -h localhost -p {DB_CONTAINER_PORT} -d odoodb_template -U odoo -f /tmp/database.zip.extract/dump.sql
# docker-compose run --rm odoo dropdb odoodb
# docker-compose run --rm odoo createdb odoodb -T odoodb_template
#
...
NO_FILESTORE=0 # do not use filestore from odoo database migration
...
function clean() {
  ...
  if [[ $PARTIAL_TEMPLATE_RESTORE == 1 ]]; then
    docker-compose run --rm odoo dropdb $PG_DB_TEMPLAT
  fi
}
...
function log_error() {
  echo -e "$(date '+%F %T') ERROR \e[1m\e[31m$*\e[0m" |& tee -a $LOG_FILE
}
...
function download_odoo_migrated_db() {
  if [[ -n $ODOO_MIGRATION_IDS ]]; then
    ...
    echo "unzip -q $tmpfile -d $tmpfile.extract"
    unzip -q "$tmpfile" -d "$tmpfile.extract" | pv -l
    mv "$tmpfile.extract/dump.sql" "$DB_FOLDER/$PG_PROD_DBNAME.sql"
    rm -Rf $FILESTORE_FOLDER/$PG_DB
    if [[ $NO_FILESTORE == 0 ]]; then
      rsync -ah --no-perms --no-owner --no-group --progress "$tmpfile.extract/filestore" "$FILESTORE_FOLDER/$PG_DB"
    fi
...
function restore_database() {
  ...
  if [[ $TEMPLATE_TO_UPDATE == 0 ]] && [[ $(docker exec -it $DB_CONTAINER_NAME psql -U odoo -lqt | grep -c $PG_DB_TEMPLATE) != 1 ]]; then
    log_error "Template database is missing, please restore it."
    exit 1
  fi
  if [[ $TEMPLATE_TO_UPDATE == 1 ]]; then
    docker-compose run --rm odoo psql -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$PG_DB_TEMPLATE' AND pid <> pg_backend_pid()"
    docker-compose run --rm odoo dropdb $PG_DB_TEMPLATE
  ...
  then
    if [ -d $FILESTORE_FOLDER/$PG_DB ]; then
      ...
      log "Restoring the container's filestore..."
      ODOO_CONTAINER_NAME=$(docker-compose ps | grep "odoo_odoo" | awk '{print $1}')
      docker cp $FILESTORE_FOLDER $ODOO_CONTAINER_NAME:/data/odoo
...
function restore_db_container() {
  ...
  until [[ $(docker inspect -f {{.State.Running}} "$DB_CONTAINER_NAME") == "true" ]]; do
      echo "Waiting for db container to be up..."
      sleep 5;
...
function usage {
    ...
    echo "  --no-filestore              Do not manage filestore when upgrading from odoo migration database"
    exit 1
}
...
prepare_folders
if [[ $RESTORE_DATABASE == 1 ]] || [[ -n $ODOO_MIGRATION_IDS ]]; then
...
[[ -n $ODOO_MIGRATION_IDS ]] && download_odoo_migrated_db
